...
\subsection{Analyse d'impact : une matrice éthique par scénario}
...
(Tab.~\ref{tab:ethique_s1}–\ref{tab:ethique_s3}) en révèle les tensions
...
\begin{figure}[H]
\centering
\screenshot{figures/capture_accueil.png}{Barre latérale de configuration de la
simulation : choix du secteur, du scénario de priorisation, du nombre de
déneigeuses, des tronçons en travaux, du fond de carte et de l'animation.}
\caption{Configuration de la simulation dans l'application.}
\label{fig:capture_accueil}
\end{figure}

\begin{figure}[H]
\centering
\screenshot{figures/capture_carte.png}{Carte interactive : réseau coloré par
priorité, marqueurs des services essentiels, déneigeuses animées (flocons) et
légende. Placez le curseur de temps en cours de tournée pour montrer
l'animation.}
\caption{Carte interactive des tournées (avec, si possible, des tronçons en
travaux pour illustrer le recalcul).}
\label{fig:capture_carte}
\end{figure}

\begin{figure}[H]
\centering
\screenshot{figures/capture_graphiques.png}{Les deux graphiques (courbe
d'accessibilité aux services essentiels dans le temps ; coût et temps de remise
en service en fonction du nombre de déneigeuses) et les deux tableaux (remise en
service par type de voie ; comparaison des trois scénarios).}
\caption{Graphiques et tableaux de résultats.}
\label{fig:capture_graphiques}
\end{figure}
