...
Pour les scénarios priorisés, nécessitant plusieurs passes successives, on couvre d'abord un sous-ensemble de rues : c'est le \emph{postier rural dirigé}, résolu par la même mécanique. Pour la \textbf{flotte} de $k$ véhicules, on applique une heuristique « route d'abord, découpe ensuite » optimisée : chaque passe est découpée en $k$ tronçons de longueur égale. Entre deux passes, les véhicules sont réaffectés de la fin de leur tronçon actuel au début du tronçon suivant en minimisant la somme des déplacements à vide grâce à un \textbf{couplage biparti de poids minimum (Algorithme Hongrois)}. Cela garantit le respect strict des priorités tout en limitant la distance parcourue à vide.
